...
class DataLoaderZipf(DataLoader):
    def __init__(self, num_files, num_samples, param, num_progs=1, operation='random'):
        ...
        for i in range(num_progs):
            ...
            pdf /= np.sum(pdf)
            # Draw samples
            self.requests += np.random.choice(files,
                                              size=num_samples, p=pdf).tolist()
            if operation == 'random':
                self.operations += np.random.choice(
                    [0, 1], size=num_samples).tolist()
            else:
                self.operations += np.full(num_samples,
                                           int(operation)).tolist()
...
class DataLoaderZipfpdf(object):
    def __init__(self, num_files, param):
        super(DataLoaderZipfpdf, self).__init__()

        self.files = np.arange(num_files)
        # Random ranks. Note that it starts from 1.
        ranks = np.random.permutation(self.files) + 1
        # Distribution
        self.pdf = 1 / np.power(ranks, param)
